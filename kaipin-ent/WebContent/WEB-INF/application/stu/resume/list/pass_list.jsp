...
//		 var data = JSON.stringify(row);
	 ...
	 */
	 function detail(times,index){
		 ...
		 location.href="<%=path%>/resume/detail?status="+status+"&resumeId="+resumeId+"&relationId="+relationId+"&userId="+userId;
	 }
...
<!-- 								<p><span>asdcasd</span><br><em>2015年10月26日18:38:41</em></p> -->
		...
		<!--邀请面试  开始-->
		<form method="POST" action="<%=path%>/resumePassOrNo/reqView.do" id="reqViewForm" name="reqViewForm">
		...
		<script type="text/javascript">
			...
			function offer(index){
				...
				$.ajax({
					 cache : false,
					 async : false,
					 url : '<%=path%>/resumeOffer/checkOffer.do',
					 type : 'POST',
					 data : {
						 relationId : row.id
					 },
								...
								$("#offer-suer").click(function () {
									...
									setTimeout(function(){
										$.ajax({
											 cache : false,
											 async : false,
											 url : '<%=path%>/resumeOffer/sendOffer.do',
											 type : 'POST',
											 contentType: "application/x-www-form-urlencoded; charset=UTF-8",
											 data : {
