	...
	<script type="text/javascript">
		...
		//简历筛选通过不通过
		 function passOrNo(oper){
			 $.ajax({                
					...
					async: false, 
					type: "POST",                
					url:  '<%=path%>/resumePassOrNo/passOrNo',                
					data:{
						ids : relationId,
						oper : oper
					},                
	 ...
	 function reqExam(){
		 $.ajax({
			 cache : false,
			 async : false,
			 url : '<%=path%>/examManager/checkExamPaper',
			 type : 'POST',
			 contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			 data : {
				 positionId : positionId
			 ...
			 success : function(data){
				 ...
				 if(datas.success){
					 $.fn.fandialog({
						...
						click:function(ok){
							if(ok){
								window.location.href="<%=path%>/examManager/init"
							}
						}
					});
				 }else{
...
<!-- 						<img src="http://kaipin.com:8083:82/12/2016/images/info/0/a2fdef36-db4e-4c25-aa89-b482b2217f06.jpg"> -->
		...
		<!--邀请面试  开始-->
		<form method="POST" action="<%=path%>/resumePassOrNo/reqView" id="reqViewForm" name="reqViewForm">
