	...
	$('#image169').cropper({
  	...
  	$("#getData").click(function(){
  		...
  		switch(operTypeCut){
			...
			case 'bg' : cutBg(); break;
			case 'logo-ent' : cutLogoEnt(datasCut); break;
		}  
  	});
  	...
  	function uploadLogoImg(data){
  		...
  		$.ajax({
			...
			success : function (data){
				var datas =eval('('+data+')');
				if(datas.success || datas.obj != ''){
					var path = datas.obj;
					if(!isNotEmpty(path)){
						path = r_path + "/static/web/university/images/fang_img.jpg";
					}
					headUrl = path;
					$("#company-logo").attr("src",path);
					$("#company-logo-head").css("background-image","url("+path+")");
					$("#editLogo").hide();
				}else{
					alert("图片保存错误");
					$("#getDataIng").hide();
  	...
  	/*更新企业logo*/
  	function uploadLogoImgEnt(data){
  		var path = data
  		$.ajax({
  			cache : false,
  			async : false,
