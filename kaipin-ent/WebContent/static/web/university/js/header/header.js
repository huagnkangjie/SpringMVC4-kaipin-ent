$(function(){
	...
	$("#account-set-btn").click(function(){
		$.ajax({                
			cache: false,                
			type: "POST",                
			url:  r_path + '/sch/getUserInfoAndConfig.do',                
			async: false, 
			data :{
				oper : 0
			},
			...
			success: function(data) {
				var datas = eval('('+data+')');
				var sex = datas.obj.user.sex;
				$("#mod-entName").val(datas.obj.info.schoolName);
				$("#mod-entName-tip").html(datas.obj.info.schoolName);
				$("#mod-email-tip").html(datas.obj.baseUser.email);
				
				$("#user-phone").val(datas.obj.baseUser.phone);
				...
				if(sex != null){
					...
					for(var key in datas.obj.user){
						var value = datas.obj.user[key];
					$("#user-"+key).val(value);
				}
...
//			});
	...
	$.fn.fandialog.methods = {
		...
		_getCenter:function($dialog){
			...
			$dialog.css({"position":"fixed","zIndex":"10000","left":left,"top":"220px"});
//			$dialog.css({"position":"fixed","zIndex":"10000","left":left,"top":top});
	...
	/**
		...
		$("#sure-modifyPw-btn").click(function(){
			window.location.href="http://kaipin.com:8083/kaipin-web/login";
//			window.location.href=r_path + "/loginController/index.do";
		...
		function getPhoneCode(phone){
			$.ajax({
				cache : false,
				type : "POST",
				url : r_path + '/phone/sendPhoneMsg.do',
				data : {
					phone : phone
				},
				async : false,
		...
		$("#save-user-info").click(function(){
			...
			var missSurname = $("#user-missSurname").val();
	...
	function getUserSelfLogo(){
		$.ajax({
			...
			success : function(data) {
				var datas = eval('('+ data +')');
				var u_type = datas.obj.u_type;
				
				var logo = datas.obj.info.schoolLogo;
				var u_type = datas.obj.u_type;
				
				
				if(logo == null || logo == ''){
					logo = r_path + "/static/web/images/headLogo.png";
				}
				
				headUrl = logo;
				r_org_id = datas.obj.info.id;
				r_uid = datas.obj.u_id;
				
				orgName = datas.obj.info.schoolShortName
				$("#company-logo-head").css("background-image","url("+logo+")");
				$("#hd-logo").css("scr",""+logo+"");
				$("#person-hd-logo").empty();
				$("#person-hd-logo").html("<img id='hd-logo' src='"+logo+"'>");
	...
	function getEntUserSelfLogo(){
		$.ajax({
			...
			success : function(data) {
				...
				var logo = datas.obj.info.logo;
				var basic_logo = logo;
				
				if(logo == null || logo == ''){
					logo = r_path + "/static/web/university/images/headLogo.png";
					basic_logo = r_path +  "/static/web/university/images/fang_img.jpg";
				...
				r_uid = datas.obj.u_id;
				debugger;
				orgName = datas.obj.info.entSimpleName;
				$("#company-logo-head").css("background-image","url("+logo+")");
				$("#company-logo-show").css("scr",""+logo+"");
				$("#person-hd-logo").empty();
				$("#person-hd-logo").html("<img id='hd-logo' src='"+logo+"' width='48' height='48' >");
				$("#company-logo-show").empty();
				$("#company-logo-show").html("<img src='"+basic_logo+"' width='108' height='108' alt='logo'>");
			}
		});
	}