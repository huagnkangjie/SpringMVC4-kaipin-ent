...
(function($){
			...
			$.ajax({                
				...
				complete: function(data) { 
					...
					for(var i = 0; i < datas.obj.feed.length; i++){
						...
						}else if(f_type == '100' || f_type == '140'){//宣讲会
	...
	function renderingMeeting(data){
		...
		if(r_org_id == f_org_id){
			delHtml = "<a href='javascript:void(0)' data-tag='"+id+"' data-rid='"+resource_id+"' class='delMeeting' >"+
...
//		//绑定点赞事件
		...
		/*删除视频*/
		$(".delMeeting").off().on("click",function(){
			var $_this = $(this);
			...
			$.fn.fandialog({
				...
				click:function(ok){
					if(ok){
						delFeed(f_id, resource_id, $_this, "meeting");
