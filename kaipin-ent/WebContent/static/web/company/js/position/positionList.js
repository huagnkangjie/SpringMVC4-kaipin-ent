function getPositionListDataStr(/*行数*/rows, companyId){
	var dataStr = "";
	$.ajax({                
		cache: true,    
		async: false, 
...
function eachPositionList(/*职位数组*/data,/*查询类型*/type,/*工作地区*/workArea,/*公司简称*/simpelEntName,/*公司logo*/logo,companyId){
	if(isNotEmpty(data)){
		var datas = eval('('+data+')')
		if(datas.rows.length == 0 || datas.rows.length == 1){
			...
			for(var i = 0; i < datas.rows.length; i ++){
				...
				url = r_path + "/position/detail?positionId=" + id + "&org_id="+companyId;
				li = li +   "<li>"+
