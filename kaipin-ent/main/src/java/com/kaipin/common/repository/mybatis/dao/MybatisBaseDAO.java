...
import com.kaipin.common.page.GenericDefaultPage;
import com.kaipin.common.page.IGenericPage;
import com.kaipin.common.exception.BeanToMapException;
import com.kaipin.common.repository.mybatis.util.BeanMapUtil;
...
public abstract class MybatisBaseDAO<T, ID extends Serializable> extends SqlSessionDaoSupport {
	...
	private static final String TOTAOL="total";
	...
	public Integer update(T ob) {
		...
		return this.update(SQL_UPDATE, ob);
	...
	public T getById(ID id) {
		return   this.getById( SQL_GETBYID, id);
	}

	
	public T getById(String name,ID id) {
		return (T) this.getSqlSession().selectOne(getSqlName(name), id);
	}
	...
	public IGenericPage<T> findPageBy(IBasePageSql sql, T param, int pageNo, int pageSize, String sort, String dir) {
		...
		Long  count = getCountBy(pageCountSql, param);
	...
	public long getCountBy(String name, T param) {
		Map<String, Object> paramMap = null;
		try {
			if (param == null) {
				paramMap = new HashMap<String, Object>();
