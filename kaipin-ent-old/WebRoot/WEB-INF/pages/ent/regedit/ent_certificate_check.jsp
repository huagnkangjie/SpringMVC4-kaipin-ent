								...
								<div class="info-details">
									...
									<div class="firstColumn fl">
										...
										<c:forEach	var="m" items="${listDoc }">
											<c:if test="${m.document_type != null &&  m.document_type != '' && m.document_type == 'business_license'}">
												<dl>
													<dt>营业执照</dt>
													<dd>
														<div class="eaxm-img">
									...
									<div class="firstColumn fl">
										<dl class="mg0 mark no-pass">
											...
											<dd>
												<c:if test="${info.verifyStatus == '1' }">
													<c:forEach var="m" items="${feedbackList }">
														<span style="color:red;">【${m.title }】</span>${m.content }</br>
													</c:forEach>
													<a href="<%=path%>/regedit/certificate.do?target=certificate&userId=${userInfo.id }&companyId=${info.id }" class="anew-sub" style="margin-top:10px;">重新提交</a>
												</c:if>
