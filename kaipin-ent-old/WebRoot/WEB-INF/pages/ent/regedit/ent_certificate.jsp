...
<html>
 <%@ include file="/WEB-INF/pages/ent/regedit/comment/regHeader.jsp" %>
	...
	<link href="<%=path%>/css/nameUnknown.css" rel="stylesheet" type="text/css" >
...
// 					location.href= r_path + "/loginController/index.do";
		...
		function doSave(){
			$.ajax({                
				...
				async: true, 
				type: "POST",                
				url:  '<%=path%>/regedit/certificateSave',                
				...
				complete: function(data) { 
					...
					if(datas.success){
						location.href="<%=path%>/regedit/certificateCheckPage.do?companyId="+companyId+"&userId="+userId;
					}else{
						alert("上传失败，请重新上传");
						$("#save").show();
						$("#saveing").hide();
		...
		<div class="sessionRegister">
			...
			<div class="submit-reg-infos">
				<div class="sri-contents">
					<form id="certificateForm" name="certificateForm" action="<%=path%>/regedit/certificateSave" method="post">
						<div class="basic-informations upload-files">
							<div class="upf-areas">
								...
								<div class="load-upf">
									<a href="javascript:void(0)" class="upf-btn select-up-img-btn" data-tag="yyzz" id="yyzzSelect">上传营业执照
										<img id="yyzzIng" src="<%=path %>/images/loading.gif" style="display:none;"/>
									</a>
									<div class="file-prv" style="display: none;" id="yyzzShowDiv">
										<img  src="<%=path %>/images/qiye_zhizhao_b.png" id="yyzzShow" onclick="imgbox.show(this.src)"alt="预览文件"/>
									</div>
								</div>
							</div>
							...
							<div class="exam-preview">
								<p>参考实例</p>
								<div class="file-prv">
									<img  src="<%=path %>/images/qiye_zhizhao_b.png"  onclick="imgbox.show(this.src)"alt="预览文件"/>
								</div>
							</div>
						...
						<div class="basic-informations upload-files">
								...
								<div class="upf-file-lists">
									<div class="load-upf">
										...
										<div class="file-prv" id="sfzShowDiv" style="display:none;">
											<img  src="<%=path %>/images/qiye_shengfen_b.jpg" id="sfzShow" onclick="imgbox.show(this.src)"alt="预览文件"/>
										</div>
									</div>
									<div class="load-upf">
										<a href="javascript:void(0)" class="upf-btn select-up-img-btn" data-tag="gzz" id="gzzSelect">上传工作证
											<img id="gzzIng" src="<%=path %>/images/loading.gif" style="display:none;"/>
										</a>
										<div class="file-prv" id="gzzShowDiv" style="display:none;">
											<img  src="<%=path %>/images/qiye_gongzuozheng_b.jpg" id="gzzShow"  onclick="imgbox.show(this.src)"alt="预览文件"/>
										</div>
									</div>
									<div class="load-upf">
										<a href="javascript:void(0)" class="upf-btn select-up-img-btn" data-tag="mp" id="mpSelect">上传名片
											<img id="mpIng" src="<%=path %>/images/loading.gif" style="display:none;"/>
										</a>
										<div class="file-prv" id="mpShowDiv" style="display:none;">
											<img  src="<%=path %>/images/qiye_mingpian_b.jpg" id="mpShow" onclick="imgbox.show(this.src)"alt="预览文件"/>
										</div>
									</div>
									<span class="clear"></span>
								</div>
							...
							<div class="exam-preview exam-preview-lists">
								...
								<div class="file-prv">
									<img  src="<%=path %>/images/qiye_gongzuozheng_b.jpg"  onclick="imgbox.show(this.src)"alt="预览文件"/>
								</div>
								<div class="file-prv">
									<img  src="<%=path %>/images/qiye_mingpian_b.jpg"  onclick="imgbox.show(this.src)"alt="预览文件"/>
								</div>
...
<!-- 						正在上传 ing -->
						...
						<input type="hidden" id="companyId" name="companyId" value="${orgId }"/>
						
						<div class="submit-box nosel-color">
							<a href="javascript:void(0)" class="reset-btn" onclick="back();">返回</a>
		...
		<script type="text/javascript">
			...
			function back(){
				location.href="<%=path%>/basicConctroller/init";
			}
		</script>
		...
		<script type="text/javascript">
					...
					$(function (){
						...
						upImg = $('#uploadImg').Huploadify({
							...
							uploader:'<%=path%>/annexController/uploadImg.do?oper=regedit&type=11&orgId=' + entId,
