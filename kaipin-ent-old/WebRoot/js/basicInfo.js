...
//							startTimeStr = startTimeStr.substring(0,startTimeStr.length-3);
						...
						if(vedioPath != null && vedioPath != ''){
							...
							vedioHtml = "<a href='javascript:void(0)' data-tags='"+name+"' class='play_video' data-src='"+vedioPath+"'>"+
											"<img src='" + r_path + "/images/play.png' width='45' height='45' title='播放'/>"+
										"</a>";
						}else{
							if(type == "1"){
								tag = "预告";
...
function returnTimer($id,$mark){
				...
				if(datas.success){
					...
					for(var i = 0; i < datas.obj.vList.length; i++){
						...
						if(!isNotEmpty(headUrl)){
							headUrl = r_path + "/images/moren_guanzhu.jpg";
						}
	...
	function showResumeDetail(uId){
		$.ajax({                
			cache: false,                
			type: "POST",                
			url:  r_path + '/resumeDetail/checkResume',   
			data:{
				uId : uId
			},
			async: false,               
			...
			success: function(data) {
				...
				if(datas.success){
					...
					var url = r_path + "/resumeDetail/init?oper=index&resumeId="+resumeId+"&userId="+uId;
