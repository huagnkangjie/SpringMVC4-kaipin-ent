...
<body>
	...
	<div class="wrap js-check-wrap">
		...
		<form class="well form-search"  name="form-search" method="post"  action="${BASE_PATH }/management/sch/main/zzlist">
			...
			<input type="text" name="entName" style="width: 200px;" value=""
				placeholder="请输入关键字..."> 
					...
					<c:forEach var="m"  items="${page.elements }" varStatus="index">
						<tr data-tag="${m.schoolName}">
							...
							<td>
								 ${m.schoolName}
								 <c:choose>
									   <c:when test="${m.schoolShortName != ''}"> <span style="color: #39C;">${m.schoolShortName}</span>
									   </c:when>
									   <c:otherwise><span style="color: #39C"><span style="color:red">未填</span></span>
									   </c:otherwise>
								</c:choose>
							</td>
							<td data-location="${m.location }">${m.location }</td>
							<td>${m.userName }</td>
							<td>${m.phone }</td>
							<td align="center">
								<c:if test="${m.verifyStatus == 0}">
									未审核
								</c:if>
								<c:if test="${m.verifyStatus == 1}">
									<span style="color:red;">不通过</span>
								</c:if>
								<c:if test="${m.verifyStatus == 2}">
									<span style="color:#CFCFCF;">已通过</span>
								</c:if>
							</td>
		
							<td>
								<a href="#" data-tag="${m.userId}" data-orgid="${m.id }" data-name="${m.schoolName}" class="show-zz">查看</a> 
							...
							<script type="text/javascript">
	 							...
	 							$(function(){ 
	 								...
	 								$(".show-zz").click(function(){
	 									...
	 									var openUrl = '${BASE_PATH}/management/sch/main/zzxq?schName='+entName+'&userId=' + id + '&schoolId=' + schoolId;//弹出窗口的url
