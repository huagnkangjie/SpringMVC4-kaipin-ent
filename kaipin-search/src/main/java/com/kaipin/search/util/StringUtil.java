...
public class StringUtil {
    ...
    public static String subString(String sArg, String cArg) {
        ...
        if (isEmpty(cArg))
            return sArg;
        return sArg.indexOf(cArg) < 0 ? sArg : sArg.substring(0, sArg.indexOf(cArg));
    ...
    public static String getHeadUrl(String head) {

        if (!StringUtils.isEmpty(head)) {

            if (!head.contains("http://") && !head.contains("https://")) {

                return SystemConfig.getInstance().getHeadUrl() + head;
