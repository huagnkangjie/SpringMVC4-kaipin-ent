package com.kaipin.search.common.page;
...
public class GenericDefaultPage<T> implements IGenericPage<T> {
    public static final int DEF_COUNT = 1000;
    /** 当前页数据列表 */
     ...
     * @see #emptyPage()
     */
    public static final GenericDefaultPage EMPTY_PAGE =
            new GenericDefaultPage(0, 0, Collections.emptyList(), 0l);
    ...
    public boolean hasNextPage() {
        ...
        if (getTotalCount() > (getPageNo() * getPageSize())) {
            return true;
        }
    ...
    public void adjustLastPageNo() {
        ...
        lastPageNo = (totalResults % getPageSize() == 0)
                ? new Double(Math.floor(totalResults / getPageSize())).intValue()
