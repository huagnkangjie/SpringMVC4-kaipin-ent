...
<%@ page language="java" import="java.util.*" pageEncoding="UTF-8"%>
	...
	<!-- 系统 -->
	<link rel="stylesheet" href="${STATIC_ENT }/css/jquery.mCustomScrollbar.css" />
	...
	<script type="text/javascript" src="${STATIC_ENT }/js/editjs.js" ></script>
	<!-- editor-->
	...
	<!-- scrollBar-->
	<script type="text/javascript" src="${STATIC_ENT }/js/scrollBar/jquery-ui-1.10.4.min.js"></script>
	<script type="text/javascript" src="${STATIC_ENT }/js/scrollBar/jquery.mousewheel.min.js"></script>
	<script type="text/javascript" src="${STATIC_ENT }/js/scrollBar/jquery.mCustomScrollbar.min.js"></script>
	<style type="text/css">
	...
	<script type="text/javascript">
	 ...
	 $(function (){
		 ...
		 datagrid =	$('#grid').datagrid({
			title : '',
			url : '<%=path%>/company/resume/datagridNoReadList.do',
			queryParams : {
				postionId : postionId,
				status : '2'
				},
			...
			columns : [ [ {
				...
				formatter:function(value,row,i){
					...
					return  htmlAcc + htmlReq + htmlExam + htmlPassOrNo + htmlOffer + htmlEnd;
				}
			} ] ],
			toolbar : '#toolbar',
	 	...
	 	function show(relationId){
	 		...
	 		$.ajax({
				 cache :false,
				 async :false,
				 url : '<%=path%>/company/resume/log.do',
				 data : {
...
//		 var data = JSON.stringify(row);
		 if(oper == 4){
			 ...
			 $.ajax({
				 ...
				 url : '<%=path%>/company/resume/oper.do',
	 			 data : {
	 				 id : row.id,
	 				 status : oper
	 			 },
	 ...
	 */
	 function detail(times,index){
		 ...
		 location.href="<%=path%>/company/resume/detail?status="+status+"&resumeId="+resumeId+"&relationId="+relationId+"&userId="+userId;
	 }
	...
	/*邀请笔试
	 ...
	 function reqExam(index){
		 ...
		 $.ajax({
			 cache : false,
			 async : false,
			 url : '<%=path%>/company/examManager/checkExamPaper.do',
			 type : 'POST',
			 contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			 data : {
			 ...
			 success : function(data){
				 ...
				 if(datas.success){
					 $.fn.fandialog({
						...
						click:function(ok){
							if(ok){
								window.location.href="<%=path%>/company/examManager/init.do"
							}
						}
					});
				 }else{
	...
	function doReqExam(index){
		...
		$.ajax({
			 cache : false,
			 async : false,
			 url : '<%=path%>/company/examManager/reqExam.do',
			 type : 'POST',
			 contentType: "application/x-www-form-urlencoded; charset=UTF-8",
			 data : {
				 positionId : row.position_id,
...
<!-- 								<p><span>asdcasd</span><br><em>2015年10月26日18:38:41</em></p> -->
		...
		<!--邀请面试  开始-->
		<form method="POST" action="<%=path%>/company/resumePassOrNo/reqView.do" id="reqViewForm" name="reqViewForm">
									...
									setTimeout(function(){
										$.ajax({
											 ...
											 data : {
												 ...
												 postionId : row_offer.position_id,
												 userId : row_offer.stu_user_id,
