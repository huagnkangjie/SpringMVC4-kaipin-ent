...
// 			 var resumeId = row.resume_id;//简历id
			 location.href="<%=path%>/company/examList/init";
		 }
		 ...
		 function editPage(oper,index){
			 ...
			 if(status == '0' && oper != '1'){
				 $.fn.fandialog({
						...
						click:function(ok){
							if(ok){
								location.href="<%=path%>/company/examManager/editPage?paperId="+paperId+"&oper="+oper;
							}
						}
					});
			 }else{
					...
					click:function(ok){
						if(ok){
							$.ajax({
								...
								async : false,
								type : 'POST',
								url : '<%=path%>/company/examManager/upOrDown.do',
								data : {
									oper : oper,
									paperId : paperId
								},
