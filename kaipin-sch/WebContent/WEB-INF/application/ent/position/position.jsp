...
<%-- 	<script type="text/javascript" src="${STATIC_ENT }/js/common-search.js" ></script> --%>
	...
	<script type="text/javascript">
		 ...
		 $(function (){
			 add = '${add}';
			 if(add == 'add'){
				 window.location.href="<%=path%>/company/position";
			 }
			 datagrid =	$('#grid').datagrid({
				url : '<%=path%>/company/position/datagrid.do',
				pagination : true,
				fitColumns : true,
				showRefresh : false,
				pageSize : 20,
		 ...
		 function count(oper){
			 $.ajax({
					...
					async : false,
					type : 'POST',
					url : '<%=path%>/company/postionCount/countPostionByEndtime.do',
					data : {
						oper : oper
					},
		 ...
		 function edit(index){
			 ...
			 location.href="<%=path%>/company/position/editPage.do?postionId="+row.id;
		 }
		 /**
		 *删除
		 */
						...
						click:function(ok){
							if(ok){
								$.ajax({
									...
									async : false,
									type : 'POST',
									url : '<%=path%>/company/position/del.do',
									data : {
										ids : row.id
									},
									success : function (data){
		 ...
		 */
		 function detail(index){
			 ...
			 var row = $("#grid").datagrid("getRows")[index];
			 var id = row.id;//简历id
			 location.href="<%=path%>/company/position/detail.do?postionId="+id;
		 }
		 ...
		 */
		 ...
		 function delCheck(ids){
			 ...
			 $.ajax({
				...
				async : false,
				type : 'POST',
				url : '<%=path%>/company/position/delCheck.do',
				data : {
					ids : ids
				},
				success : function (data){
		...
		<div class="dateTime-recruitmentInfo" style="margin-top: 88px;">
