...
(function($){
	...
	var hasload = "0MB";
	var timer;//全局轮训
	var progerss = 0; // 0初始化代表不做任何操作，1代表在上传中
	var timeProgerss;
	
	$.fn.Huploadify = function(opts){
		var itemTemp = '<div class="showDiv"><div id="${fileID}" class="uploadify-queue-item"><div id="fileDiv" class="uploadify-progress"><div class="uploadify-progress-bar"></div></div><br><span class="up_filename" title="${fileName}">文件名：${fileName}</span><a href="javascript:void(0);" class="uploadbtn">上传</a><a href="javascript:void(0);" class="delfilebtn">文件解析中...</a></div></div>';
		var defaults = {
			...
			var uploadManager = {
				...
				_renderFile : function(file){
					...
					if(option.showUploadedSize){
						var num = '<div class="up_size"><span class="progressnum"><span class="uploadedsize">已上传 0MB</span>/<span class="totalsize">${fileSize}</span></span></div>'.replace(/\${fileSize}/g,F.formatFileSize(file.size));
						$html.find('.uploadify-progress').after(num);
					}
					
					//判断是否显示上传百分比	
					if(option.showUploadedPercent){
						var percentText = '<br><span class="up_percent">0%</span>';
				  	...
				  	if(xhr.upload){
						...
						xhr.onreadystatechange = function(e) {
							if(xhr.readyState > 2){
								...
								var data = getUploadValue();
								if(data.obj != null){
									hasload = data.obj.hasLoad + "MB";
									percent = data.obj.percent + "%";
								}
								
//								timer = window.setTimeout(getUploadValue,1000);
//								var datasArray = datas.split(",");
//								var dataStr = datasArray[datasArray.length-1];
...
//								var hasLoad = data[1];
								...
								//var percent = percent +'%';
								if(option.showUploadedSize){
									eleProgress.nextAll('.progressnum .uploadedsize').text(F.formatFileSize(4));
									eleProgress.nextAll('.progressnum .totalsize').text(F.formatFileSize("共"));
								}
								if(option.showUploadedPercent){
									eleProgress.nextAll('.up_percent').text(percent);
									$(".uploadedsize").html(hasload);
								}
								eleProgress.children('.uploadify-progress-bar').css('width',percent);
							}
							if(xhr.readyState == 4){
								...
								progerss = 0;//恢复默认状态
								//清除轮训
								window.clearInterval(timeProgerss);
						...
						if(file.status===0){
							...
							clearProgress();//清楚缓存
							progerss = 1;
							var timer1 = setTimeout(runProgress(),1000);
						}
	...
	function runProgress(){
		timeProgerss = setInterval(function(){
			if(progerss == 1){
				var data = getUploadValue();
				var datas = eval('('+data+')');
				if(datas.obj != null){
					hasload = datas.obj.hasLoad + "MB";
					percent = datas.obj.percent + "%";
					console.log(hasload + " " + percent);
				}
	...
	function getUploadValue(){
		...
		$.ajax({                
			...
			async: false, 
			type: "POST",                
			url:  r_path + '/annexProcess/process.do',          
			data:{
			},              
