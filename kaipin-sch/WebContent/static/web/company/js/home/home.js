			...
			success: function(data) {
				...
				if(datas.success){
					...
					for(var i = 0; i < datas.obj.list.length; i++){
						...
						var path = r_path + "/company/resume/detail?relationId="+relationId;
...
//						$("#resumeDatas").html("人才库（"+datas.obj.counts+"）");//收到简历总数
					...
					$(".show-resume").click(function(){
						...
						location.href= r_path + "/company/resume/detail?status=0&resumeId="+resumeId+"&relationId="+relationId;
					});
	...
	function getResumeCount(){
		$.ajax({
			 cache : false,
			 async : true,
			 url : r_path + '/company/resume/counts.do',
			 data : {
