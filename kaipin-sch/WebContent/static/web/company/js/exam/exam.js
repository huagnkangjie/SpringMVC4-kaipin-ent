...
$(function(){
	...
	$("#writer-manager").click(function(){
		$.ajax({
			...
			async : false,
			type : 'POST',
			url : r_path + '/company/postionCount/countPostionByEndtime.do',
			data : {
				oper : 1
			},
	...
	function getPositionList(){
		$.ajax({                
			...
			async: false, 
			type: "POST",                
			url:  r_path + '/company/examManager/positionList',                
			data:{
				page : 1,
				rows : 50
			},              
	...
	$("#addPage").click(function(){
		...
		if(positionId != ''){
			location.href = r_path + "/company/examManager/addPage?positionId=" + positionId;
		}else{
			$.fn.fandialog({
				title:"提示信息",
				contents: "请选择职位",
		...
		click:function(ok){
			if(ok){
				$.ajax({                
					...
					async: false, 
					type: "POST",                
					url:  r_path + '/company/examList/passOrNo.do',                
					data:{
						oper : obj,
						inviteId : inviteId
					},              
					...
					success: function(data) {
						var datas = eval('('+data+')');
						if(datas.success){
							window.location.href=r_path + "/company/examList/init.do?postionId="+positionId+"&positionName=" + positionName;
						}else{
							alert("操作失败");
						}
					}            
