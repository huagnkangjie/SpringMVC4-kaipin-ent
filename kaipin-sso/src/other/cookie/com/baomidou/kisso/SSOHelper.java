...
public class SSOHelper {
	...
	public static void addCookieRWNoSecure(HttpServletResponse response, String name, String value) {
		...
		int maxAge = SSOHelper.getKissoService().getConfig().getCookieMaxage();

		CookieHelper.addCookie(response, domain, path, name, value, maxAge, false, false);
...
public static String getStringToken(HttpServletRequest request){
	
	String cookieName=SSOHelper.getKissoService().getConfig().getCookieName();
	
	Cookie token = CookieHelper.findCookieByName(request, cookieName);
